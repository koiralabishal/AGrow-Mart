...
export const addAgriInput = async (req, res) => {
  try {
    ...
    if (!req.file) {
      return res.json({
        success: false,
        message: "Agricultural Input Image is required",
      });
    }
    ...
    if (req.file.size > 1024 * 1024 * 2) {
      return res.json({
        success: false,
        message: "Product image must be less than 2MB",
      });
    }
...
export const getAgriInputsBySupplierEmail = async (req, res) => {
  ...
  try {
    ...
    const fertilizers = agriInputs.filter(
      (input) => input.category === "fertilizers"
    );
    ...
    const formattedSuppliers = await Promise.all(
      suppliersWithInputs.map(async (supplier, index) => {
        ...
        if (supplierUser && supplierUser.profilePic) {
          ...
          if (
            typeof supplierUser.profilePic === "object" &&
            supplierUser.profilePic.path
          ) {
            // If path starts with '/', prepend the server URL (BACKWARD COMPATIBILITY)
            if (supplierUser.profilePic.path.startsWith("/")) {
              // For Vercel, this won't really work for old images, but keeping logic sane
        ...
        } else if (supplier.sampleInput.image) {
          // Fallback to input image if no profile pic
          if (
            supplier.sampleInput.image.path &&
            supplier.sampleInput.image.path.startsWith("http")
          ) {
            profilePicUrl = supplier.sampleInput.image.path;
          } else if (supplier.sampleInput.image.name) {
            // Fallback for old local images
            profilePicUrl = `/uploads/products/${supplier.sampleInput.image.name}`;
          }
        }
